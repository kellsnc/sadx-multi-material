...
// Multi Material
float TexStageCount : register(c100);
float4 TexStageColorOp : register(c101);
float4 TexStageEnvMap : register(c102);
...
#define VS_TEX_STAGE(id) \
    if (TexStageCount > id) \
	{ \
		if (TexStageEnvMap[id] == 1) \
	    { \
	        output.uv##id = (float2) mul(float4(input.normal, 1), wvMatrixInvT); \
	        output.uv##id = (float2) mul(float4(output.uv##id, 0, 1), TextureTransform); \
	    } \
	    else \
	    { \
	        output.uv##id = input.tex; \
	    } \
	} \
    else \
        output.uv##id = float2(0.0f, 0.0f); \
...
PS_IN vs_main(VS_IN input)
{
	...
	output.uv0 = (float2)mul(float4(input.normal, 1), wvMatrixInvT);
	output.uv0 = (float2)mul(float4(output.uv0, 0, 1), TextureTransform);
#else
    output.uv0 = input.tex;
#endif
...
#define PS_TEX_STAGE(id) \
	if (TexStageCount > id) { \
		if (TexStageColorOp[id] == 2) \
			result = tex2D(tex##id, input.uv##id); \
		else if (TexStageColorOp[id] == 4) \
			result  *= tex2D(tex##id, input.uv##id); \
		else if (TexStageColorOp[id] == 7) \
			result  += tex2D(tex##id, input.uv##id); \
		else if (TexStageColorOp[id] == 10) \
			result  -= tex2D(tex##id, input.uv##id); \
	} \
